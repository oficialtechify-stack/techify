...
// 20-step palette of Techify emerald, lime, and cyber-green tones matching the site.
// If the darkest color is used, alpha=0 => total transparency in darkest areas.
const siteGreenColors = [
  [0.0, 0.02, 0.01],
  [0.0, 0.04, 0.02],
  [0.0, 0.07, 0.03],
  [0.01, 0.11, 0.04],
...
function buildFragmentShader(): string {
  ...
  // Convert siteGreenColors array to GLSL array of vec3.
  const colorArraySrc = siteGreenColors.map((c) => `vec3(${c[0]}, ${c[1]}, ${c[2]})`).join(",\n  ");
...
// 20-step palette of site greens.
vec3 greenColors[NUM_COLORS] = vec3[](
  ${colorArraySrc}
);
...
void main() {
  ...
  vec3 colLow = greenColors[iLow];
  vec3 colHigh = greenColors[iHigh];
